...
import { BookOpen, User, GraduationCap, ShieldCheck, ArrowRight, ArrowLeft } from 'lucide-react'; // Added ArrowRight
...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-gray-50 p-4 lg:p-8 animate-page-enter">
      ...
      {/* 1. BACKGROUND BUBBLES (Identical to Login) */}
        ...
        {/* Back Button */}
          ...
          <div className="text-center mb-8">
            <div className="flex justify-center mb-4">
               {/* Same Icon, but maybe rotate it slightly to show action? Kept standard for now */}
          ...
          {/* Role Selector (I want to join as...) */}
          <div className="mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input type="text" placeholder="Jose" required className="py-5 px-4 rounded-lg border-gray-300 focus:ring-1 focus:ring-[#A51C30] focus:border-[#A51C30] transition-all bg-white" />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-900 ml-1 font-serif uppercase tracking-wide">Last Name</label>
                <Input type="text" placeholder="Rizal" required className="py-5 px-4 rounded-lg border-gray-300 focus:ring-1 focus:ring-[#A51C30] focus:border-[#A51C30] transition-all bg-white" />
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold text-gray-900 ml-1 font-serif uppercase tracking-wide">Email Address</label>
              <Input type="email" placeholder="jose@pup.edu.ph" required className="py-5 px-4 rounded-lg border-gray-300 focus:ring-1 focus:ring-[#A51C30] focus:border-[#A51C30] transition-all bg-white" />
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold text-gray-900 ml-1 font-serif uppercase tracking-wide">Password</label>
              <Input type="password" required className="py-5 px-4 rounded-lg border-gray-300 focus:ring-1 focus:ring-[#A51C30] focus:border-[#A51C30] transition-all bg-white" />
            </div>

            <Button 
              type="submit" 
              isLoading={isLoading}
              className="w-full py-6 bg-[#A51C30] text-white border border-[#A51C30] hover:bg-[#851626] hover:border-[#851626] font-serif font-bold text-lg rounded-lg shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 mt-2"
            >
              Create Account
            </Button>
          </form>
          ...
          <div className="mt-6 text-center text-sm font-serif">
            ...
            <Link to="/auth/login" className="font-bold text-[#A51C30] hover:underline hover:text-black transition-all inline-flex items-center gap-1">
              Sign in <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
        </div>

        {/* RIGHT SIDE - IDENTICAL TO LOGIN (For smooth UX) */}
